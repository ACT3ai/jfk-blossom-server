...
router.delete<CommonState>("/:hash", async (ctx, next) => {
  ...
  if (!(await blobDB.hasBlob(sha256))) throw new HttpErrors.NotFound("Blob does not exist");

  const pubkey = ctx.state.auth.pubkey;

  if (await blobDB.hasOwner(sha256, pubkey)) {
    await blobDB.removeOwner(sha256, pubkey);

    if (config.storage.removeWhenNoOwners && (await blobDB.listOwners(sha256)).length === 0) {
      ...
      await forgetBlobAccessed(sha256);
    }
  }
